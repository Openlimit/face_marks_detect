...
REGISTER_OP("FarthestPointSampleX")
  .Input("inp: float32")
  .Input("npoint: int32")
  .Output("out: int32")
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ...
    ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(dims1, 0), c->Dim(dims2,0)});
    c->set_output(0, output);
    return Status::OK();
  });
...
class FarthestPointSampleXGpuOp : public OpKernel {
public:
    explicit FarthestPointSampleXGpuOp(OpKernelConstruction *context) : OpKernel(context) {}

    void Compute(OpKernelContext *context) override {
        const Tensor &npoint = context->input(1);
        int m = npoint.shape().dim_size(0);

        const Tensor &inp_tensor = context->input(0);
        OP_REQUIRES(context, inp_tensor.dims() == 3 && inp_tensor.shape().dim_size(2) == 3,
                    errors::InvalidArgument("FarthestPointSampleX expects (batch_size,num_points,3) inp shape"));
        int b = inp_tensor.shape().dim_size(0);
        int n = inp_tensor.shape().dim_size(1);
        auto inp_flat = inp_tensor.flat<float>();
        const float *inp = &(inp_flat(0));
...
REGISTER_KERNEL_BUILDER(Name("FarthestPointSampleX").Device(DEVICE_GPU), FarthestPointSampleXGpuOp);
