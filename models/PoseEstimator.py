...
class PoseEstimator(object):
    def __init__(self, setting):
        ...
        with self.graph.as_default():
            ...
            poseNet = PointCNN(points_sampled_fps, features_sampled_fps, self.is_training, setting, scope='posenet')
            self.predicts = poseNet.predicts
                ...
                if (batch_idx_train % setting.step_val == 0 and (batch_idx_train != 0 or args.load_ckpt is not None)) \
                    ...
                    for batch_val_idx in range(batch_num_val):
                        ...
                        for i, data_name in enumerate(name_batch):
                            ...
                            rxyz, rotation = pf.get_rotation(rotation_range=setting.rotation_range,
                        ...
                        error = np.mean(np.sum(np.abs(labels_batch - predicts), axis=-1))
                        avg_loss += loss
                        avg_err += error
