...
class PartBoxDetector(object):
    def __init__(self, setting):
        ...
        with self.graph.as_default():
            ...
            partbox = PointCNN(points_sampled_fps, features_sampled_fps, self.is_training, setting, scope='partbox',
                               out_shape=(setting.label_dim // 3, 3))
    ...
    def train(self, args):
        ...
        with self.graph.as_default():
            ...
            labels = tf.placeholder(tf.float32, shape=(None, setting.label_dim // 3, 3), name='labels')
            loss_op = tf.losses.huber_loss(labels, self.predicts, delta=10.0)
            ...
            for batch_idx_train in range(batch_num):
                if (batch_idx_train % setting.step_val == 0 and (batch_idx_train != 0 or args.load_ckpt is not None)) \
                    ...
                    for batch_val_idx in range(batch_num_val):
                        ...
                        points_batch = np.zeros((batch_size_val, setting.origin_num, setting.data_dim),
                                                dtype=np.float32)
                        labels_batch = np.zeros((batch_size_val, setting.label_dim // 3, 3), dtype=np.float32)
                        for i, data_name in enumerate(name_batch):
                            ...
                            labels_batch[i] = data_utils.get_part_box(points, landmarks)
                        ...
                        error = np.mean(np.sqrt(np.sum((labels_batch - predicts) ** 2, axis=-1)))
                ...
                labels_batch = np.zeros((batch_size_train, setting.label_dim // 3, 3), dtype=np.float32)
                for i, data_name in enumerate(name_batch):
