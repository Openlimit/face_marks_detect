...
from tf_ops.group import tf_grouping
from tf_ops.sampling import tf_sampling


def farthest_point_sample_x(points, npoints):
    fps_idx = tf_sampling.farthest_point_sample_x(npoints, points)
    batch_indices = tf.tile(tf.reshape(tf.range(tf.shape(points)[0]), (-1, 1, 1)),
                            (1, tf.shape(npoints)[0], 1))
    indices = tf.concat([batch_indices, tf.expand_dims(fps_idx, -1)], axis=-1)
...
def farthest_point_sample(points, npoints):
    ...
    batch_indices = tf.tile(tf.reshape(tf.range(tf.shape(points)[0]), (-1, 1, 1)), (1, npoints, 1))
    indices = tf.concat([batch_indices, tf.expand_dims(fps_indices, -1)], axis=-1)

    return indices


def ball_query_indices(queries, points, k, radius=5):
    queries_shape = tf.shape(queries)
    batch_size = queries_shape[0]
    point_num = queries_shape[1]

    indices_q, _ = tf_grouping.query_ball_point(radius, k, points, queries)
    batch_indices = tf.tile(tf.reshape(tf.range(batch_size), (-1, 1, 1, 1)), (1, point_num, k, 1))
    indices = tf.concat([batch_indices, tf.expand_dims(indices_q, axis=-1)], axis=-1)
...
def positive_uniform(bound):
    return bound * random.random()
...
def get_xform(rotation_range=(0, 0, 0, 'u'), scaling_range=(0.0, 0.0, 0.0, 'u'), order='rxyz'):
    rx = rotation_angle(rotation_range[0], rotation_range[3])
    ry = rotation_angle(rotation_range[1], rotation_range[3])
    rz = rotation_angle(rotation_range[2], rotation_range[3])
    rotation = euler2mat(rx, ry, rz, order)
    ...
    xform = scaling * rotation
    rotation = rotation

    return xform, rotation


def get_rotation(rotation_range=(0, 0, 0, 'u'), order='rxyz'):
    rx = rotation_angle(rotation_range[0], rotation_range[3])
    ry = rotation_angle(rotation_range[1], rotation_range[3])
    rz = rotation_angle(rotation_range[2], rotation_range[3])
    rotation = euler2mat(rx, ry, rz, order)
    return [rx, ry, rz], rotation


def get_rotation_mtx(rx, ry, rz, order='rxyz'):
    rotation = euler2mat(rx, ry, rz, order)
    return rotation


def get_transform_mtx(rotation, centroid):
    # (PxR)T = RTxPT
...
def transform_points(points, transform):
    points_q = np.ones((points.shape[0], 4), dtype=np.float32)
    points_q[:, :3] = points
    points_q = np.dot(transform, points_q.T)
    return points_q[:3].T
...
def conv2d(input, output, name, is_training, kernel_size, stride=(1, 1),
           reuse=None, with_bn=True, activation=tf.nn.elu):
    conv2d = tf.layers.conv2d(input, output, kernel_size=kernel_size, strides=stride, padding='VALID',
                              activation=activation,
...
def conv3d(input, output, name, is_training, kernel_size, strides=(1, 1, 1), padding='SAME',
           reuse=None, with_bn=True, activation=tf.nn.elu, num_in_channels=None):
    """
        不能用tf.layers.conv3d,因为input的channel可能是动态的
    """
    with tf.variable_scope(name):
        ...
        kernel = tf.get_variable('weights',
                                 shape=[kernel_d, kernel_h, kernel_w, num_in_channels, output],
                                 initializer=tf.glorot_normal_initializer(),
                                 regularizer=tf.contrib.layers.l2_regularizer(scale=1.0),
                                 dtype=tf.float32)

        stride_d, stride_h, stride_w = strides
        conv3d = tf.nn.conv3d(input, kernel,
        ...
        if not with_bn:
            ...
            conv3d = tf.nn.bias_add(conv3d, biases)
        if activation is not None:
            conv3d = activation(conv3d)
        return batch_normalization(conv3d, is_training, name + '_bn', reuse) if with_bn else conv3d


def conv3d_layers(input, output, name, is_training, kernel_size, strides=(1, 1, 1),
                  reuse=None, with_bn=True, activation=tf.nn.elu):
    conv3d = tf.layers.conv3d(input, output, kernel_size=kernel_size, strides=strides, padding='SAME',
                              activation=activation,
                              kernel_initializer=tf.glorot_normal_initializer(),
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=1.0),
                              reuse=reuse, name=name, use_bias=not with_bn)
